...
typedef NS_ENUM(NSUInteger, SPPageControlAlignment) {
	SPPageControlAlignmentLeft = 1,
	SPPageControlAlignmentCenter,
	SPPageControlAlignmentRight
};

typedef NS_ENUM(NSUInteger, SPPageControlVerticalAlignment) {
	SPPageControlVerticalAlignmentTop = 1,
	SPPageControlVerticalAlignmentMiddle,
	SPPageControlVerticalAlignmentBottom
};

typedef NS_ENUM(NSUInteger, SPPageControlTapBehavior) {
	SPPageControlTapBehaviorStep	= 1,
	SPPageControlTapBehaviorJump
};

@interface SPPageControl : UIControl
...
@property (nonatomic) SPPageControlAlignment alignment					UI_APPEARANCE_SELECTOR; // deafult is Center
@property (nonatomic) SPPageControlVerticalAlignment verticalAlignment	UI_APPEARANCE_SELECTOR;	// deafult is Middle
...
@property (nonatomic) SPPageControlTapBehavior tapBehavior;	// SPPageControlTapBehaviorStep provides an increment/decrement behavior exactly like UIPageControl. SPPageControlTapBehaviorJump allows specific pages to be selected by tapping their respective indicator. Default is SPPageControlTapBehaviorStep
...
// SPPageControl mirrors UIPageControl's standard accessibility functionality by default.
// Basically, the accessibility label is set to "[current page index + 1] of [page count]".

// SPPageControl extends UIPageControl's functionality by allowing you to name specific pages. This is especially useful when using
