...
typedef NS_ENUM(NSUInteger, SPPageControlImageType) {
	SPPageControlImageTypeNormal = 1,
	SPPageControlImageTypeCurrent,
	SPPageControlImageTypeMask
};

typedef NS_ENUM(NSUInteger, SPPageControlStyleDefaults) {
	SPPageControlDefaultStyleClassic = 0,
	SPPageControlDefaultStyleModern
};

static SPPageControlStyleDefaults _defaultStyleForSystemVersion;

@interface SPPageControl ()
...
+ (void)initialize
{
	...
	if ([currSysVer compare:reqSysVer options:NSNumericSearch] != NSOrderedAscending) {
		_defaultStyleForSystemVersion = SPPageControlDefaultStyleModern;
	} else {
		_defaultStyleForSystemVersion = SPPageControlDefaultStyleClassic;
	}
}
...
{
	_numberOfPages = 0;
	_tapBehavior = SPPageControlTapBehaviorStep;
...
#else
	[self setStyleWithDefaults:SPPageControlDefaultStyleClassic];
#endif
	
	_alignment = SPPageControlAlignmentCenter;
	_verticalAlignment = SPPageControlVerticalAlignmentMiddle;
...
- (CGFloat)_leftOffset
{
	...
	CGFloat left = 0.0f;
	switch (_alignment) {
		case SPPageControlAlignmentCenter:
			left = ceilf(CGRectGetMidX(rect) - (size.width / 2.0f));
			break;
		case SPPageControlAlignmentRight:
...
- (CGFloat)_topOffsetForHeight:(CGFloat)height rect:(CGRect)rect
{
	...
	switch (_verticalAlignment) {
		case SPPageControlVerticalAlignmentMiddle:
			top = CGRectGetMidY(rect) - (height / 2.0f);
			break;
		case SPPageControlVerticalAlignmentBottom:
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex type:(SPPageControlImageType)type
{
	if (pageIndex < 0 || pageIndex >= _numberOfPages) {
		return;
	}
	...
	switch (type) {
		case SPPageControlImageTypeCurrent:
			dictionary = self.currentPageImages;
			break;
		case SPPageControlImageTypeNormal:
			dictionary = self.pageImages;
			break;
		case SPPageControlImageTypeMask:
...
{
    [self _setImage:image forPage:pageIndex type:SPPageControlImageTypeNormal];
	[self _updateMeasuredIndicatorSizes];
}

- (void)setCurrentImage:(UIImage *)image forPage:(NSInteger)pageIndex
{
	[self _setImage:image forPage:pageIndex type:SPPageControlImageTypeCurrent];;
	[self _updateMeasuredIndicatorSizes];
}

- (void)setImageMask:(UIImage *)image forPage:(NSInteger)pageIndex
{
	[self _setImage:image forPage:pageIndex type:SPPageControlImageTypeMask];
...
- (id)_imageForPage:(NSInteger)pageIndex type:(SPPageControlImageType)type
{
	if (pageIndex < 0 || pageIndex >= _numberOfPages) {
		return nil;
	}
...
{
	return [self _imageForPage:pageIndex type:SPPageControlImageTypeNormal];
}

- (UIImage *)currentImageForPage:(NSInteger)pageIndex
{
	return [self _imageForPage:pageIndex type:SPPageControlImageTypeCurrent];
}

- (UIImage *)imageMaskForPage:(NSInteger)pageIndex
{
	return [self _imageForPage:pageIndex type:SPPageControlImageTypeMask];
}
...
- (void)setStyleWithDefaults:(SPPageControlStyleDefaults)defaultStyle
{
	switch (defaultStyle) {
		case SPPageControlDefaultStyleModern:
			...
			break;
		case SPPageControlDefaultStyleClassic:
		default:
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (SPPageControlTapBehaviorJump == self.tapBehavior) {
